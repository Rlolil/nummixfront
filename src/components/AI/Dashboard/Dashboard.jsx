...
const Dashboard = () => {
  ...
  return (
    <div className="container mx-auto px-2 py-4 text-gray-800 dark:bg-zinc-800 dark:text-white ">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {data.stats.map((s, i) => (
          <div
            key={i}
      ...
      <div className="bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl shadow-sm p-5 mb-6">
        <h2 className="text-[16px] font-semibold mb-2">{t("pages.ai.dashboard.alerts.title")}</h2>
        <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">{t("pages.ai.dashboard.alerts.subtitle")}</p>

        {data.alerts.map((a, i) => (
          <div
            key={i}
            className={`border-l-4 ${a.borderColor} ${a.bgColor} dark:bg-zinc-700 dark:border-zinc-600 p-4 mb-4 rounded-lg`}
          >
            <h3 className="font-semibold text-gray-800 dark:text-white flex items-center gap-2 flex-nowrap min-w-0">
              <span className="truncate">{a.title}</span>
              <span className={`${a.badgeColor} text-xs px-2 py-0.5 whitespace-nowrap rounded-full`}>{a.impact}</span>
            </h3>
            <p className="text-gray-600 dark:text-gray-300 text-sm mt-2">{a.text}</p>
          </div>
        ))}
      </div>

      <div className="bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl shadow-sm p-5">
        <h2 className="text-[16px] font-semibold mb-2">{t("pages.ai.dashboard.aiResults.title")}</h2>
        <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">{t("pages.ai.dashboard.aiResults.subtitle")}</p>

        <div className="space-y-6">
          {data.aiAnalysis.map((r, i) => (
            <div key={i} className="flex justify-between items-start border-l-4 border-blue-400 dark:border-blue-600 pl-4">
              <div>
                <h3 className="font-semibold text-gray-800 dark:text-white">{r.title}</h3>
                <p className="text-gray-600 dark:text-gray-300 text-sm mt-1">{r.text}</p>
              </div>
              <span className={`${r.badgeColor} text-xs px-3 py-1 rounded-full whitespace-nowrap flex-shrink-0`}>{r.impact}</span>
            </div>
          ))}
        </div>
      </div>
