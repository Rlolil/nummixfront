            ...
            <div key={idx} className="p-4 border border-gray-200 rounded-lg">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-gray-900 mt-2">{it.start} - {it.end}</p>
                ...
                <div className="flex items-center gap-2">
                  <button className="p-2 hover:bg-gray-100 rounded-md" title={t('common.edit', { ns: 'translation', defaultValue: 'Edit' })} onClick={() => openEdit(it, idx)}>
        ...
        {editItem && (
          <div>
            ...
            <div role="dialog" aria-modal="true" className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-51 w-full max-w-md bg-white border border-gray-200 rounded-lg shadow p-6">
              ...
              <h3 className="text-lg font-semibold mb-4">{t('pages.hr.portal.leave.history.editTitle', { defaultValue: 'Edit Leave' })}</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-sm mb-1">{t('pages.hr.leave.modal.leaveType', { defaultValue: 'Leave type' })}</label>
                  <select className="w-full border rounded-md px-3 py-2 text-sm" value={editItem.type} onChange={(e) => setEditItem(r => ({ ...r, type: e.target.value }))}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm mb-1">{t('pages.hr.leave.modal.startDate')}</label>
                    <input type="date" className="w-full border rounded-md px-3 py-2 text-sm" value={editItem.start} onChange={(e) => setEditItem(r => ({ ...r, start: e.target.value }))} />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">{t('pages.hr.leave.modal.endDate')}</label>
                    <input type="date" className="w-full border rounded-md px-3 py-2 text-sm" value={editItem.end} onChange={(e) => setEditItem(r => ({ ...r, end: e.target.value }))} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm mb-1">{t('pages.hr.leave.common.days', { defaultValue: 'Days' })}</label>
                    <input type="number" min="1" className="w-full border rounded-md px-3 py-2 text-sm" value={editItem.days} onChange={(e) => setEditItem(r => ({ ...r, days: Number(e.target.value) }))} />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">{t('pages.hr.leave.requests.status.label', { defaultValue: 'Status' })}</label>
                    <select className="w-full border rounded-md px-3 py-2 text-sm" value={editItem.status} onChange={(e) => setEditItem(r => ({ ...r, status: e.target.value }))}>
                ...
                <div>
                  <label className="block text-sm mb-1">{t('pages.hr.leave.modal.reason', { defaultValue: 'Reason' })}</label>
                  <input className="w-full border rounded-md px-3 py-2 text-sm" value={editItem.description} onChange={(e) => setEditItem(r => ({ ...r, description: e.target.value }))} />
                </div>
              </div>
