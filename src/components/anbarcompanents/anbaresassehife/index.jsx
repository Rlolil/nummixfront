...
export default function AnbarEsasSehife() {
    ...
    if (loading) return <div className="p-8 text-[#7D8597]">{t('common.loading')}</div>;

    return (
        <div className="min-h-screen bg-[#FFFFFF] dark:bg-[#001233] p-4 lg:p-6 xl:p-8 space-y-8">
            {/* Başlıq */}
            <div>
                <h1 className="text-xl lg:text-2xl font-bold dark:text-white text-[#023E7D]">{t('pages.warehouse.dashboard.title')}</h1>
                <p className="text-[#7D8597] text-sm lg:text-base">{t('pages.warehouse.dashboard.subtitle')}</p>
            </div>
            ...
            <div className="border border-red-200 dark:bg-[#001233] dark:border-[#979DAC] bg-red-50 rounded-xl p-4 lg:p-6 shadow-sm">
                    ...
                    {minStock.map((item, i) => (
                        <div key={i}>
                            <div className="flex flex-col lg:flex-row lg:justify-between text-sm font-medium">
                                <div>
                                    {item.name}{" "}
                                    <span className="text-xs bg-[#979DAC] text-white px-2 py-0.5 rounded">{item.code}</span>
                                </div>
                            ...
                            <div className="w-full bg-[#979DAC] h-2 rounded-full mt-1">
                                <div
...
function StatCard({ title, value, subtitle, color, icon }) {
    ...
    return (
        <div className="bg-[#FFFFFF] dark:bg-[#001233] dark:border-[#979DAC] dark:text-white border border-[#33415C] rounded-xl p-4 lg:p-5 shadow-sm hover:shadow-md transition">
            <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium dark:text-white text-[#5C677D]">{title}</h3>
                {icon && <div className={`text-xl ${colorMap[color] || "text-gray-400"}`}>{icon}</div>}
            </div>
            <p className="text-xl lg:text-2xl font-bold mt-1 dark:text-white text-[#023E7D]">{value}</p>
            {subtitle && (
