  ...
  return (
    <div className="border border-[#33415C] space-y-4 p-3 sm:p-4 my-4 rounded-lg shadow-sm bg-[#FFFFFF] text-[#001233]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
        <div>
          <p className="text-lg sm:text-xl font-semibold text-[#023E7D]">{t('pages.accounting.financialReports.balanceSheet.title')}</p>
          <p className="text-[#7D8597] text-sm sm:text-base">{t('pages.accounting.financialReports.common.asOf', { date: 'September 30, 2025' })}</p>
        </div>
        <div>
          <svg
      ...
      <div>
        <h3 className="text-base sm:text-lg font-semibold text-[#023E7D]">{t('pages.accounting.financialReports.balanceSheet.assets.title')}</h3>
          ...
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {currentAssets.map((row, i) => (
                <tr key={`ca-${i}`} className="border-b border-[#979DAC]">
                  ...
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('CA', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
          ...
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {nonCurrentAssets.map((row, i) => (
                <tr key={`nca-${i}`} className="border-b border-[#979DAC]">
                  <td className="text-left p-1 sm:p-2">{row.label}</td>
                  <td className="text-right p-1 sm:p-2">{fmt(Number(row.amount))}</td>
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('NCA', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
      ...
      <div>
        <h3 className="text-base sm:text-lg font-semibold text-[#023E7D]">{t('pages.accounting.financialReports.balanceSheet.liabilities.title')}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-[#979DAC]">
            ...
            <tbody>
              {currentLiabilities.map((row, i) => (
                <tr key={`cl-${i}`} className="border-b border-[#979DAC]">
                  <td className="text-left p-1 sm:p-2">{row.label}</td>
                  <td className="text-right p-1 sm:p-2">{fmt(Number(row.amount))}</td>
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('CL', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
      ...
      <div>
        <h3 className="text-base sm:text-lg font-semibold text-[#023E7D]">{t('pages.accounting.financialReports.balanceSheet.equity.title')}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm">
            <thead>
              <tr className="border-b border-[#979DAC]">
            ...
            <tbody>
              {equityRows.map((row, i) => (
                <tr key={`eq-${i}`} className="border-b border-[#979DAC]">
                  ...
                  <td className="text-right p-1 sm:p-2">{fmt(Number(row.amount))}</td>
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('EQ', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
