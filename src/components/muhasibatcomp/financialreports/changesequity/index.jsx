  ...
  return (
    <div className="border border-[#33415C] dark:border-[#979DAC] space-y-4 p-3 sm:p-4 my-4 rounded-lg shadow-sm bg-[#FFFFFF] dark:bg-[#002855] text-[#001233] dark:text-white">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
        <div>
          <p className="text-lg sm:text-xl font-semibold text-[#023E7D] dark:text-[#89A4D6]">{t('pages.accounting.financialReports.equityChanges.title')}</p>
            ...
            {totalsRow.map((r, i) => (
              <tr key={`eq-${i}`} className="border-b border-[#979DAC] dark:border-[#33415C] hover:bg-[#0453A4]/10 dark:hover:bg-[#0453A4]/30">
                ...
                <td className="p-1 sm:p-2 text-right">
                  <button onClick={() => setEditing({ index: i, label: r.label, shareCapital: String(r.shareCapital), retainedEarnings: String(r.retainedEarnings) })} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] dark:border-[#979DAC] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
                  <button onClick={() => { const c = t('common.confirmDeleteRow', { defaultValue: 'Sətir silinsin?' }); if (window.confirm(c)) setRows(prev => prev.filter((_, idx) => idx !== i)); }} className="inline-flex items-center gap-1 px-2 py-1 border rounded text-red-600 border-red-200 dark:border-red-400/40 dark:hover:bg-red-900/30 hover:bg-red-50"><FaTrash /></button>
                </td>
              </tr>
            ))}
        ...
        <div className="p-6">
          ...
          <div className="fixed top-1/2 left-1/2 z-51 w-full max-w-md -translate-x-1/2 -translate-y-1/2 bg-[#FFFFFF] dark:bg-[#002855] rounded-lg shadow-lg p-6">
            <div className="flex justify-between items-center border-b border-[#979DAC] dark:border-[#33415C] pb-3">
              ...
              <button onClick={() => setEditing(null)} className="text-[#7D8597] hover:text-[#001233] dark:hover:text-white text-xl">×</button>
            </div>
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" className="mt-1 w-full border border-[#33415C] dark:border-[#979DAC] rounded-md px-3 py-2 dark:bg-[#001F3F] dark:text-white" value={editing.shareCapital} onChange={(e) => setEditing(prev => ({ ...prev, shareCapital: e.target.value }))} />
                </div>
                <div>
                  ...
                  <input type="number" className="mt-1 w-full border border-[#33415C] dark:border-[#979DAC] rounded-md px-3 py-2 dark:bg-[#001F3F] dark:text-white" value={editing.retainedEarnings} onChange={(e) => setEditing(prev => ({ ...prev, retainedEarnings: e.target.value }))} />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2 pt-4 border-t border-[#979DAC] dark:border-[#33415C] mt-4">
              <button onClick={() => setEditing(null)} className="px-4 py-2 border border-[#33415C] dark:border-[#979DAC] rounded-md hover:bg-[#0453A4] hover:text-white transition-colors">{t('common.cancel', { defaultValue: 'Cancel' })}</button>
