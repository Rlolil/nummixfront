  ...
  return (
    <div className="border border-[#33415C] space-y-4 p-3 sm:p-4 my-4 rounded-lg shadow-sm bg-[#FFFFFF] text-[#001233]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
        <div>
          <p className="text-lg sm:text-xl font-semibold text-[#023E7D]">{t('pages.accounting.financialReports.cashFlow.title')}</p>
          ...
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {operatingRows.map((row, i) => (
                <tr key={`op-${i}`} className="border-b border-[#979DAC]">
                  ...
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('OP', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
          ...
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {investingRows.map((row, i) => (
                <tr key={`inv-${i}`} className="border-b border-[#979DAC]">
                  <td className="text-left p-1 sm:p-2">{row.label}</td>
                  <td className="text-right p-1 sm:p-2">{fmt(Number(row.amount))}</td>
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('INV', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
          ...
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody>
              {financingRows.map((row, i) => (
                <tr key={`fin-${i}`} className="border-b border-[#979DAC]">
                  <td className="text-left p-1 sm:p-2">{row.label}</td>
                  <td className="text-right p-1 sm:p-2">{fmt(Number(row.amount))}</td>
                  <td className="text-right p-1 sm:p-2">
                    <button onClick={() => openEdit('FIN', i, row)} className="inline-flex items-center gap-1 px-2 py-1 border border-[#33415C] rounded text-[#0466CB] hover:bg-[#0453A4] hover:text-white transition-colors mr-2"><FaEdit /></button>
