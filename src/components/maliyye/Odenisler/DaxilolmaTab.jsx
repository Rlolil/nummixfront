  ...
  return (
    <div className="rounded-xl">
      <div className='block md:hidden text-[16px] mb-5'>
        <h1 className='font-semibold text-[#001233] dark:text-white'>{t('pages.finance.payments.inflows.title')}</h1>
        <h1 className='text-[#7D8597] dark:text-[#7D8597]'>{t('pages.finance.payments.inflows.subtitle')}</h1>
      </div>

      <div className="hidden md:block p-5 overflow-x-auto border border-[#979DAC] dark:bg-[#002855] rounded-xl bg-white dark:border-[#33415C]">
          ...
          <tbody className="text-sm">
            {odenisData.map((item, i) => (
              <tr key={i} className="border-b border-[#979DAC] dark:border-[#33415C] hover:bg-gray-50 dark:hover:bg-[#0453A4]/20 transition">
                ...
                <td className="py-3 text-[#5C677D] dark:text-[#7D8597]">{item.cat}</td>
                <td className="py-3 text-[#5C677D] dark:text-[#7D8597]">{item.date}</td>
                <td className="py-3 text-[#5C677D] dark:text-[#7D8597]">{item.status}</td>
                <td className="py-3 text-right font-medium text-[#001233] dark:text-white">{item.amount}</td>
                <td className="py-3 text-right text-[#5C677D] dark:text-[#7D8597]">{item.emeliyyat}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {odenisData.map((item, i) => (
          <div key={i} className="border border-[#979DAC] dark:border-[#33415C] rounded-xl p-4 shadow-sm bg-white dark:bg-[#33415C]">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[#5C677D] dark:text-[#7D8597] text-sm">{t('pages.supplier.orders.table.columns.supplier')}:</span>
            ...
            <div className="flex justify-between items-center mb-2">
              <span className="text-[#5C677D] dark:text-[#7D8597] text-sm">{t('pages.supplier.common.dueDate')}:</span>
              <span className="text-[#001233] dark:text-white">{item.date}</span>
            </div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-[#5C677D] dark:text-[#7D8597] text-sm">{t('pages.supplier.orders.table.columns.status')}:</span>
            ...
            <div className="flex justify-between items-center">
              <span className="text-[#5C677D] dark:text-[#7D8597] text-sm">{t('pages.finance.payments.actions.title', 'Actions')}:</span>
