...
const Categories = () => {
    ...
    return (
        <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow border border-gray-200 dark:border-gray-700 mt-7 mx-auto">
            <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-1">{t('pages.finance.budgeting.categoriesTitle')}</h2>
            <p className="text-gray-500 dark:text-gray-400 mb-6">{t('pages.finance.budgeting.categoriesSubtitle')}</p>

            {data.map((item, index) => (
                <div key={index} className="mb-6">
                    <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-3">
                            <h3 className="text-gray-800 dark:text-gray-100 font-medium">{item.title}</h3>
                            ...
                            {item.qenaet && (
                                <span className="text-green-800 dark:text-green-300 border border-green-600 text-xs font-semibold px-2 py-0.5 rounded-sm"
                                    style={{ backgroundColor: 'rgba(16, 185, 129, 0.2)' }}>
                                    {item.qenaet}
                                </span>
                            )}
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        {item.qaliq && (
                            <span className="text-green-700 dark:text-green-400 text-xs font-semibold px-2 py-0.5 rounded-full">
                                {item.qaliq} AZN {t('pages.finance.budgeting.labels.remaining')}
                            </span>
                        )}
                    </div>
